...
function leftCategory() {
    // 1. 拿到相应的标签
    let parentDom = document.getElementsByClassName('category_main_left')[0];
    let childDom = parentDom.getElementsByClassName('category_main_left_con')[0];
    // 2. 求出父标签和子标签的高度
    let parentH = parentDom.offsetHeight;
    let childH = childDom.offsetHeight;
    // console.log(parentH, childH);
    // 3. 确定合理的滚动区间
    let maxY = 0; // 最大滚动区间
    let minY = -(childH - parentH);  // 最小滚动的区间
    // console.log(minY);
    // 4. 确定合理的缓冲区间
    let buffer = 150;
    //5. 设置过渡 清除过渡  改变位置
    ...
    mjd.tap(childDom, function (e) {
        ...
        if(distanceY > minY){
         ...
         // 7.5 模拟数据的加载
         let rightDom = document.getElementsByClassName('category_main_right')[0];
